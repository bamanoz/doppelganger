...
The current memory implementation has a strong canonical core and a deliberately narrow retrieval path:

- `packages/extension-memory/src/service.ts` owns actor/project partitioning, immutable revisions, receipts, evidence, conflicts, FTS5, canonical revalidation, stable-profile selection, and two-source reciprocal-rank fusion.
- `packages/extension-memory/src/protocol.ts` turns whole canonical revisions into context contributions. Preferences receive instruction authority; all other kinds receive data authority. The outer `ContextProtocol` applies the final hard budget.
- `packages/extension-memory/src/capture.ts` consumes only completed `turn-committed` lifecycle events, but extraction and candidate writes currently run inline in the subscriber. Stable candidate operation IDs already derive from `(deliveryId, ordinal)`.
- `packages/extension-memory/src/projection-store.ts` and `packages/extension-memory-vectors/src/coordinator.ts` implement identifier-only durable vector work, leases, retries, generation rebuild, atomic activation, and canonical hit revalidation.
...
- Make committed-turn extraction durable, restart-safe, idempotent, candidate-only, and fail-open to host work.
...
- Keep current memory tools compatible and keep omission of every new Loader row behaviorally valid.
...
`MemoryService` remains the only canonical mutation owner and the package root remains responsible for canonical tools, retrieval, and automatic recall. New derived behavior is mounted through ordinary optional Cordis plugins in the same package:
...
All state is stored through the existing `doppelgangerInstanceSqlite` database opened by `MemoryService`; optional plugins inject `doppelgangerMemory` and use its transaction-owned canonical database seam. They do not open a second canonical database or create a second lifecycle transport.
...
- retrieval remains canonical L2 plus FTS5 and any configured semantic L2 index;
...
### 3. Add additive canonical-database tables but keep them non-canonical

Bump the memory schema from version 4 to version 5 in one transaction. Existing canonical tables and identifiers remain unchanged. New tables are visibly separated by purpose:
...
- `memory_context_projection_work`: identifier-only jobs for revision, group, and relation projection with lease/lease/retry state.
- `memory_extraction_jobs`: committed delivery identity, partition/session/turn identity, bounded filtered principal and assistant material, state, lease/retry state, expiry.
...
- `memory_working_checkpoint_work`: identifier-only checkpoint jobs with leaseable retry state.
...
Foreign keys are used wherever deletion semantics permit. Identifier-only remote deletion work remains valid after canonical content deletion, so it continues to avoid content-bearing foreign keys.
...
Content-bearing extraction jobs and working turns use explicit expiry columns. Successful extraction deletes principal/assistant job material after stable candidate rows have been delivered; terminal failure deletes it at bounded expiry. Superseded working turns and checkpoints are deleted immediately when no longer needed. The design does not claim encryption at rest; instance-root filesystem protection remains an operator responsibility.
...
Alternative: store queues in a separate database. Rejected because canonical mutation plus work enqueue must share one SQLite transaction.
...
- `retrieve(request): MemoryRetrievalCandidate[]` gathers and revalidates candidates without presentation budgeting.
- `planContext(request, candidates): MemoryContextPlan` chooses authority class, category, tier, and ordering under the memory provider's budget.
- `recordContextUsage(receipt)` persists accepted canonical revision usage idempotently.

Retrieval branches are:

1. canonical stable profile;
2. original-query FTS5;
3. optional expanded-query FTS5 branches;
...
Each branch produces ranks only. Deterministic weighted reciprocal-rank fusion combines them. Before planning, every candidate is reloaded through the existing actor, scope, status, temporal, record, and current-revision checks. Group and relation expansion occurs before this canonical revalidation, never after it.

The original complete normalized principal turn always drives FTS5. The existing bounded semantic query projection remains only the semantic input. Query expansion has a separate short deadline and count/character limits; failure records a sanitized diagnostic and contributes no branch.
...
The capture listener performs only bounded validation/filtering and a short SQLite transaction:

1. ignore non-completed or incomplete committed turns;
2. strip recursive memory text and reject credential-bearing material;
3. insert one `memory_extraction_jobs` row keyed by `(instance, actor, deliveryId)` with bounded payload and expiry;
4. return without awaiting extraction.

A serialized worker uses leases and bounded exponential backoff. Its state machine is:
...
### 11. Keep relations local, revision-bound, and one-hop

The first implementation stores relations only in canonical SQLite derived tables. It does not require graph support from vector backends. Relation projector output is validated against:
...
Add root tool `memory.pipeline.status`, always available with canonical memory. It reports bounded stage summaries for retrieval, planning, usage, extraction, working context, local projections, hierarchy, and relations. Optional coordinators report into `memory_pipeline_health`; queue counts are read directly from SQLite. Fields are restricted to counts, durations, budget totals, tier counts, generation IDs, queue states, fallback counts, zero-result counts, and last failure category/time.
...
### 14. Migration and rollout are staged by optional composition

Implementation order and operational rollout:

1. Add schema version 5 and the context acceptance callback while preserving current default behavior.
2. Refactor retrieval into candidate gathering and planning; prove L2-only output remains authority- and budget-compatible.
3. Add local projection generation and enable `context-engine` with a test projector in generated disposable presets.
4. Convert capture to the durable queue; existing omission and `enabled: false` remain disabled.
5. Add working-context, usage feedback, hierarchy, relations, expansion, and diagnostics.
6. Generalize vector identity/adapters with default `record-l2` only.
7. Enable additional vector projection kinds only after local projection generation is healthy and rebuilt.

Schema migration is forward-only. Disabling optional rows rolls behavior back to canonical L2/FTS5 without deleting canonical state, but running an older binary against a version-5 database is unsupported; operational rollback requires restoring the pre-migration database backup. Semantic rollback remains generation-scoped and cannot reinterpret incompatible vector spaces or projection profiles.
...
- **Durable turn payloads increase privacy exposure.** Capture and working context retain only bounded filtered material, apply secret rejection before persistence, use expiry, and erase payloads after terminal work. This still requires protected instance storage.
