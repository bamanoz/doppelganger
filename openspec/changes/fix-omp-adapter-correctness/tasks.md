...
- [x] 1.2 Add failing scenarios proving fresh ephemeral context is resolved before every model request in one agent run, preserves original outbound messages, avoids duplicate projection, and fails open without stale context.
...
## 3. Per-Request Context and Lifecycle Correctness

- [x] 3.1 Restrict `before_agent_start` to ensuring the current binding and creating binding-local principal input and turn identity without modifying the OMP system prompt.
- [x] 3.2 Project assembled runtime context through OMP's per-request `context` hook as one new ephemeral agent-attributed developer text message, resolving every invocation and returning original messages unchanged on empty or failed resolution.
...
- [x] 4.1 Update `docs/hosts/oh-my-pi.md` to document immutable per-session child bindings, post-commit rebinding behavior, same-session tree retention, per-model-request ephemeral context, binding-owned lifecycle identities, and neutral disposal semantics.
- [x] 4.2 Update `docs/architecture/protocols.md` only if needed to clarify the existing host-neutral rule that context is resolved at each host model-request boundary, without adding OMP hooks or optional capabilities to the baseline protocol.
