...
39. every Runtime Session owns one bounded router for ordinary Cordis logs, while exporter omission and shipped `standard` produce no destination output or persistent log history;
40. independently composed file and Sentry rows apply exact filters, bounded queues, contained failure, transactional reload, idempotent disposal, and cross-session isolation without adding runtime-owned configuration fields;
41. rolling JSONL output uses explicit absolute operator-owned paths, complete records, deterministic size rotation and retention, and a one-process-writer-per-path invariant;
42. Sentry resolves one named environment credential per generation, uses only a private client and scope, and bounds shutdown without mutating host-global Sentry state;
43. OMP keeps stdout exclusively for framed JSON-RPC, retains stderr for emergency diagnostics, and adds no ordinary-log RPC, callback, capability, report, tool, context, or UI route.
