...
### 4. Resolve one context snapshot per agent run

`before_agent_start` ensures the current binding, creates one active portable turn, resolves its context, and appends non-empty content to the existing system prompt:

```text
turnId = <binding.sessionId>:turn:<binding-local ordinal>
principalInput = event.prompt
started = false
contextRequestId = <binding.sessionId>:context:<binding-local ordinal>
```

OMP preserves the returned system-prompt override for every model continuation after tools and clears it when the agent run ends. The adapter therefore calls `context.resolve` exactly once for the direct user input. Identity, traits, memory, reminders, and other stable providers cannot be re-injected after each tool call, and provider changes become visible at the next user turn.

The hook preserves OMP's existing system prompt and appends only the authority-aware assembler's flattened accepted content. Empty resolution returns no override. If resolution fails, the captured binding is failed only if it is still current; stale failure cannot disable a replacement binding and no previous context is reused.

Alternative considered: project through OMP's per-request `context` message transform. Rejected because one user turn can contain several tool-driven model requests; recomputing stable Persona and memory guidance at each continuation duplicates semantic input and changes the turn after work has begun.
...
- **Per-turn system-prompt semantics:** OMP's `before_agent_start` result is scoped to one agent run, retained across its model continuations, and cleared afterward. Tests must exercise that actual lifecycle and prove only one child context request occurs after tool use.
...
1. Extend the OMP extension test harness to support mutable session ID/CWD, multiple child connections, controllable pending activation/disposal, and invocation of turn and transition hooks.
2. Add failing behavioral scenarios for one context snapshot across tool continuations, session switch/branch/tree behavior, stale callbacks and closures, replacement failure, shutdown races, and neutral lifecycle ordering.
...
6. Resolve context in `before_agent_start`, append it to the run-scoped system prompt, and remove the per-request `context` projection path.
