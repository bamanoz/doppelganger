...
- `host-extension-runtime` owns the versioned Host Extension catalog, normalized ordered selection plan, and fresh protected-composition instantiation. DSH builds one immutable catalog from the standard `runtime-host` and `actor` definitions plus explicitly trusted DSH-only definitions, validates selections before Fiber creation, and never accepts authored Runtime Preset control over that catalog;
- `extension-protocols` owns the actor-neutral shared Runtime Host bridge with a frozen closed capability profile, revisioned tool snapshots, exact invocation, protected approval grants, cancellation, declared lifecycle availability, and a transport-independent conformance suite. The standard `runtime-host` Host Extension is the only entry that attaches it;
...
- DSH Actor Identity is the independently selected standard `actor` Host Extension. Default namespaced anonymous-home binding, explicit bound/unbound and trusted-resolver modes, and provider absence remain distinguishable and cannot be supplied by Runtime Presets, Persona, the `runtime-host` Host Extension, or the shared bridge;
- DSH-specific services or events remain typed host-admitted Host Extension definitions in the same protected composition. Their factories receive only closed bounded JSON-compatible session facts, instantiate fresh session-isolated entries, own cleanup as Cordis effects, expose no raw DSH runtime objects, and make no security-sandbox claim;
- Doppelganger Dynamic Runtime Plugins remain a separate portable Runtime Preset feature and must not route through `@deepseek-ai/dsh-cordis-host-runner`; the future product dependency closure must make their Loader package resolvable without making generic `host-dsh` depend on its semantics;
