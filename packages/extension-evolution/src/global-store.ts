...
export class GlobalEvolutionStore {
  ...
  mutate(command: EvolutionMutationCommand, context: EvolutionMutationContext): EvolutionProposal {
    ...
    return this.database.transaction(storage => {
      ...
      const proposal = applyMutation(this.list(), command, context)
      this.persist(storage, proposal)
      storage.prepare(`
  ...
  private fromRow(row: Record<string, unknown>): EvolutionProposal {
    ...
    return validateStoredProposal(kind === 'persona'
      ? deepFreeze({ ...common, kind, scope: 'global' as const })
      : deepFreeze({ ...common, kind, scope: 'global' as const }))
  }

  private persist(storage: InstanceSqliteDatabase, proposal: EvolutionProposal): void {
    storage.prepare(`
    ...
    `)
    for (const evidence of proposal.evidence) evidenceInsert.run(
    ...
    const transitionInsert = storage.prepare(`
      INSERT OR IGNORE INTO evolution_transitions(
        instance_id, actor_id, proposal_id, id, from_status, to_status, detail, source_ids_json, created_at
      ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
    `)
    for (const transition of proposal.history) transitionInsert.run(
    ...
    const reminderInsert = storage.prepare(`
      INSERT OR IGNORE INTO evolution_reminders(
        instance_id, actor_id, proposal_id, id, session_id, turn_id, created_at
      ) VALUES (?, ?, ?, ?, ?, ?, ?)
    `)
    for (const reminder of proposal.reminders) reminderInsert.run(
