...
let nextProbeCall = 0
const ASYNC_TRANSITION_TIMEOUT_MS = 10_000
...
async function currentProbe(
  ...
  const deadline = Date.now() + ASYNC_TRANSITION_TIMEOUT_MS
...
describe('OpenClaw catalog and reload boundaries', () => {
  ...
  it('preserves audited rollback and rejects stale native closures', { timeout: 30_000 }, async () => {
    const binding: ReloadDisposalBinding = {
