...
The network-free root gate runs every workspace typecheck and test, verifies one Cordis installation, validates package dependencies/imports against `scripts/package-boundaries.json`, and runs documentation, legacy-contract, focused-spec ownership, executable-evidence, and repository-source integrity checks. The boundary checker derives edges from TypeScript syntax (imports, side-effect and type-only imports, re-exports, and literal dynamic imports), attributes named subpaths to their owning workspace package, rejects cross-package relative imports, and ignores comments and ordinary strings. Computed dynamic imports remain outside this static edge policy. Every helper imported by an executable repository command must exist as tracked source in a clean checkout; ignored or untracked local copies cannot satisfy the gate.
...
The shared matrix exercises OMP's actual Actor Identity absence, explicit unbound state, and bound state through the real child transport; OpenClaw proves its fixed-profile unbound and bound states through the direct adapter. Backend maintenance overlap fixtures own and close their controlled adapter instances; SQLite's synchronous exclusive operation is probed reentrantly at the actual PRAGMA boundary.

## OpenClaw evidence

The implementation target is `openclaw@2026.9.1`. The install candidate's published build SHA is `ad6fe23aecb9b833d68139b0ddc9f239b894d2f1`; the separate source investigation used commit `837e0b20f479f4fa060bd7a2d50112e279103fb8`. These identifiers are not interchangeable, and source inspection alone is not native runtime proof.

Package-local tests own preparation atomicity, public exports, configuration, direct activation, fixed-catalog projection, context authority, exact actor routing, approval input/revision binding, cancellation, reload fencing, empty lifecycle capability, and exhaustive disposal. The native certification target is:

```sh
npx vitest run --root packages/host-openclaw tests/native-smoke.spec.ts
```

The smoke prepares an artifact with awaited initial MCP discovery and builds a clean isolated dependency closure: private workspace TypeScript package trees are copied under `<artifact>/runtime-packages`, their package names are symlinked from the artifact's `node_modules` so resolved source realpaths remain outside `node_modules`, and external JavaScript dependencies stay under `node_modules`. It then installs the artifact through OpenClaw's plugin manager, starts the actual Gateway with conversation hooks permitted, drives two native sessions through a deterministic model provider, resolves native allow-once and deny requests, and shuts the Gateway down. Acceptance requires observed instruction/data projection, direct and MCP tool calls, denied-call absence, undeclared-tool absence, two isolated Runtime Sessions, no fabricated `turn-committed`, and disposal of both sessions and MCP processes. This source-checkout closure is not evidence of a published or compiled private package.

**Recorded outcome, September 6, 2026:** the exact installed-Gateway target passed against `openclaw@2026.9.1` build `ad6fe23`. The full `host-openclaw` suite passed 14 files and 56 tests, including artifact preparation/install, two isolated native sessions, awaited MCP discovery, instruction/data context, direct and MCP tool calls, allow-once and deny approval, cancellation/cleanup, absent undeclared names, empty lifecycle publication, and graceful Gateway shutdown. A preceding parallel workspace run required forced shutdown once; the immediate exact rerun and subsequent full host suite completed gracefully, so the smoke remains mandatory regression evidence.

The same integration gate observed 76 Composition Runtime tests, 98 `host-omp` tests, 14 real linked/project-local OMP package tests, 50 `extension-mcp` tests, seven Host Extension runtime tests, and the exact OpenClaw native smoke passing. This certifies the implemented adapter for the pinned OpenClaw package/build; another OpenClaw version or build requires its own evidence.
...
- OpenClaw host changes: run the package-local host suite, then the installed-Gateway native target above against exactly `openclaw@2026.9.1`. Prove staged preparation preserves prior output on failure; runtime selection matches the prepared preset; non-bundled conversation hooks are enabled; the 10-second default host warmup is not confused with MCP's default 60-second startup deadline or cleanup completion; synchronous factories expose only ready prepared names; context preserves instruction/data authority; actor binding requires an exact configured route; every declared tool consumes one exact native pre-dispatch record carrying binding, call, optional run, name, revision, and catalog-generation correlation; ordinary tools accept valid native middleware argument changes; approval-required calls additionally revalidate the final input digest and require genuine allow-once; abort/disposal cancels active calls; catalog drift withdraws stale tools without adding undeclared names; reset/end/shutdown dispose isolated bindings; and no unsupported external harness, automatic capture, arbitrary generated native name, or lifecycle event is claimed.
...
- MCP import changes: prove synchronous structural validation, non-blocking Plugin and Composition Runtime activation in default `background` mode, opt-in `await-ready` initial-generation success/failure/cancellation with exhaustive teardown, independent per-server startup, exact operator-authored command/arguments or endpoint, initialization and discovery timeout classification, atomic initial commit, retained tools after non-fatal refresh failure, atomic withdrawal after transport closure, background accepted in-place updates in both modes, unchanged-generation retention, stale late-result rejection, and exhaustive cleanup during spawn, initialization, discovery, refresh, and active invocation. Add a real OMP child vertical that observes the initially absent tool appear through the ordinary dynamic catalog path; do not add MCP-specific host code or UI.
...
The current reviewed security run reports four unresolved high-severity dependency entries involving `@huggingface/transformers`, `adm-zip`, `onnxruntime-node`, and `sharp`. This is not a clean security result; it is acceptable only while those exact reviewed entries continue to match the repository baseline and deployment restriction.
