# Persona Extension Specification
...
### Requirement: Composed Persona behavior survives extraction
...
#### Scenario: Actor-aware Persona composition remains operational
- **ID**: `persona.integration.actor-aware`
...
- **WHEN** a generated full-stack Runtime Preset activates through the generic OMP adapter with a configured host actor binding
- **THEN** its Persona Loader rows contribute configured identity and ordered traits without actor configuration while composed storage, lexical memory, and semantic memory retain their configured behavior
