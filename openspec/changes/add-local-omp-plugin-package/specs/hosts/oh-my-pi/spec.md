...
#### Scenario: Development actor is supplied externally
- **ID**: `omp.package.external-actor-binding`
- **EVIDENCE**: `packages/omp/tests/plugin-package.spec.ts::binds only a non-empty externally configured development actor`
- **WHEN** OMP starts with `DOPPELGANGER_ACTOR_ID=valera`
- **THEN** the plugin passes `valera` as the immutable OMP actor binding without storing it in the package, Runtime Preset, project manifest, or patch
...
- **EVIDENCE**: `packages/omp/tests/plugin-package.spec.ts::contains the resolvable dependency closure for shipped standard`
...
- **EVIDENCE**: `packages/omp/tests/plugin-package.spec.ts::dogfoods Mark through the delegated repository extension`
- **WHEN** OMP starts in the Doppelganger repository with the documented development environment
- **THEN** Mark activates through the same package entrypoint used by local plugin linking and the project extension contains no `dev/doppelganger`, `valera`, `child.ts`, or `createDoppelgangerOmpExtension` configuration
