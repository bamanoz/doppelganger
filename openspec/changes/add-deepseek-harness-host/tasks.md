...
- [ ] 4.4 Cover authority projection, hard-budget truncation, stable multi-step turn input, complete-prompt governance, and runtime-context suppression
...
- [ ] 8.1 Add a hermetic DSH vertical scenario that activates the shipped actor-neutral `standard` Runtime Preset shape already exercised by OMP, plus an actor-aware Mark scenario, and observes context, native tools, actor binding, lifecycle, persistence, reload, and teardown end to end
