...
- Add one explicitly writable `trait:evolving-profile` to the development and current user `mark` Runtime Presets and compose Persona Authoring after Persona and tools. `identity.md`, `engineer`, `concise`, shipped `standard`, and every undeclared asset remain read-only.
...
- Modified development assets: `dev/doppelganger/.runtime-presets/mark/runtime.cordis.yml` plus `traits/evolving-profile.md`; shipped `presets/standard` remains unchanged.
