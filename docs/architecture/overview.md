...
Doppelganger does not implement an agent loop or model provider. Persona, memory, Evolution, CodeGraph code intelligence, Dynamic Runtime Plugins, and MCP tool import are optional extension layers, not kernel concepts.
...
- `extension-protocols` owns context, revisioned tools, approval/cancellation, lifecycle, independent Actor Identity, the closed Runtime Host API, and its transport-independent adapter conformance suite;
...
- Persona, scoped Persona Authoring, memory, local embedding, vector backends, Dynamic Runtime Plugins, Evolution, CodeGraph, and MCP import are optional extension layers;
...
- `host-omp` depends only on generic composition, protocol, and Runtime Preset seams, transports the shared bridge over one child connection, and remains Persona-, storage-, Evolution-, CodeGraph-, MCP-, and generated-plugin-neutral.
