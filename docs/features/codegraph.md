...
`codegraph.status` accepts an empty object. Without a workspace it returns a normalized workspace-unavailable diagnosis and starts no process. Otherwise it discovers and validates the binary, runs `codegraph status <workspace> --json`, validates required fields and bounds, ignores unknown additive fields, and returns binary, index, pending-change, build/extraction, state, root, and `explorationSafe` diagnostics. Missing or incompatible binaries are diagnostic states; malformed, failed, timed-out, or oversized status execution is a structured tool failure.
