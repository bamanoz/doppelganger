...
Before every model request, including repeated requests inside one agent run, the adapter resolves current assembled runtime context with the owning binding's principal input and turn identity. It appends one ephemeral agent-attributed developer text message to a new outbound message array without modifying OMP's retained messages or system prompt. Empty or failed resolution returns the original outbound messages unchanged, so prior context is never reused as stale fallback. Runtime tool JSON Schemas are validated and translated to OMP parameter schemas. Dynamic proxies are replaced exactly after reload; removed definitions become inactive, and each closure remains bound to the generation and canonical descriptor that registered it. Actor selection or switching is never projected as a tool, and memory schemas expose no actor field.
...
OMP lifecycle observations are normalized and forwarded to the runtime-side plugin using identities captured by the owning binding, never values reread from mutable OMP session state. Turn and compaction ordinals are binding-local; session, turn, call, and delivery IDs therefore remain internally consistent across switch or branch callbacks. `tool_execution_end` is the sole source for bounded `tool-completed` results and errors. Final assistant content from `turn_end` becomes `turn-committed`; aggregate `turn_end.toolResults` are not copied or synthesized into lifecycle events. Callbacks that settle after their binding is detached are discarded. OMP's resumable `agent_end` and `session_stop` hooks do not fabricate `session-completed`; only committed turns are candidate-capture input. These generic context, tool, lifecycle, approval, and actor-binding paths do not require Persona or memory extensions.
...
An opt-in Evolution Loader row follows the same generic path. OMP projects all seven portable `evolution.*` controls with its normal `doppelganger_` name transformation, contributes policy and reminder context at model-request resolution, exactly removes proxies after row removal, and requires no Evolution-specific adapter code. Proposal controls are non-executing and declare no native approval; any later Persona revision or generated-code execution retains the owning tool's separate approval contract.
...
Invalid selection/configuration, activation failure, fatal transport failure, runtime failure, or child failure disables Doppelganger for the affected binding, reports a visible diagnostic, and leaves OMP usable. A contained notification-observer rejection reports its diagnostic but does not disable the runtime. The process owner preserves the concrete bounded child-exit reason for an in-flight RPC instead of replacing it with a generic transport closure. Replacement and shutdown use the same detach-first neutral disposal order: projected tools are withdrawn, `session-disposed` is published for the captured owning session when possible, Runtime Session disposal is requested, transport is closed, and child termination escalates when cleanup does not settle. `session_shutdown` synchronously closes commit eligibility and schedules that exhaustive bounded teardown through the coordinator, so its short OMP handler neither waits for cleanup nor permits a pending activation to commit afterward. Cleanup diagnostics and non-graceful outcomes remain visible.
