...
### 10. Opt in through explicit Runtime Preset policy; keep standard read-only

A user Runtime Preset may add an `evolving-profile` trait and select it as a normal Persona asset. A Loader row then enables only `trait:evolving-profile` in Persona Authoring. This policy is authored with the preset and is exercised in generated temporary fixtures rather than a personal development preset.

Identity and undeclared traits remain inspectable but not writable. The shipped actor-neutral `standard` preset does not compose Persona Authoring and remains unchanged. Runtime Preset copy semantics are unchanged: copying a preset copies its complete assets and authored capability policy.
