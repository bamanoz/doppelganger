...
import { cp, mkdir, mkdtemp, readFile, readlink, readdir, realpath, rm, writeFile } from 'node:fs/promises'
...
async function materializeOpenClawPeer(artifact: string): Promise<readonly string[]> {
  const source = join(sourceModules, 'openclaw')
