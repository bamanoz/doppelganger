...
- [ ] 1.5 Implement memory schema version 5 with additive projection, extraction, working-context, usage, relation, and bounded health tables and indexes
- [ ] 1.6 Add version-4-to-version-5 migration tests covering transaction rollback, preserved canonical identifiers/content, and rejection of malformed new state
- [ ] 1.7 Extend hard deletion to remove every local derived row and preserve identifier-only remote deletion work before canonical rows disappear
- [ ] 1.8 Add deletion tests covering presentation tiers, groups, relations, usage, queues, checkpoints, embedding cache, semantic markers, and repeated deletion

## 2. Retrieval and Progressive Context Planning

- [ ] 2.1 Refactor `MemoryService.search` into reusable canonical candidate gathering plus the existing tool-compatible L2 result budgeting
- [ ] 2.2 Preserve stable-profile, lexical, semantic, reciprocal-rank, subject-diversity, canonical-revalidation, and failure-fallback behavior through the retrieval refactor
...
- [ ] 3.1 Implement context projection generation identities covering projector identity and presentation, hierarchy, and relation format versions
- [ ] 3.2 Implement atomic publication and lookup of validated current L0/L1 revision tiers with canonical record/revision provenance
...
- [ ] 3.4 Implement leaseable idempotent local projection work for revision, subject-group, and relation jobs with bounded attempts and sanitized failure categories
- [ ] 3.5 Enqueue configured revision projection work transactionally from remember, promotion, correction, conflict resolution, pin-relevant updates, and deletion paths
...
- [ ] 4.1 Replace inline capture work with a bounded listener transaction that deduplicates completed committed deliveries and persists filtered extraction jobs
...
- [ ] 5.1 Implement the optional working-context plugin with bounded committed-turn retention by count, token estimate, session partition, and expiry
- [ ] 5.2 Reuse recursive-memory and credential filtering before persisting any working-turn material
- [ ] 5.3 Enqueue immutable turn-range checkpoint jobs only when configured thresholds are exceeded and a checkpoint projector is available
...
- [ ] 5.7 Add `memory-working-context.spec.ts` coverage for compaction thresholds, canonical/tool separation, partition isolation, failure fallback, disposal, expiry, and omission neutrality

## 6. Usage Feedback and Pipeline Diagnostics

- [ ] 6.1 Persist idempotent usage receipts only from final accepted memory source IDs with exact session, turn, record, and revision identity
- [ ] 6.2 Maintain bounded per-revision usage rollups and cleanup retention without storing query or contribution content
...
- [ ] 7.4 Generalize canonical projection work loading, delivery, deletion, indexed markers, rebuild paging, and canonical source revalidation by projection kind
...
- [ ] 7.6 Rebuild and atomically activate all configured projection kinds only after per-kind expected/current/stale verification succeeds
...
- [ ] 8.1 Add generated disposable Runtime Presets that exercise independent omission and activation of capture, context-engine, working-context, projector, embedder, vector index, and coordinator rows in matching isolation realms
- [ ] 8.2 Add Loader tests for strict config rejection, missing projector methods, duplicate services, row removal, reload cutover, stale workers, and exhaustive Fiber disposal
...
- [ ] 8.8 Run package-local typechecks and focused tests while iterating, then run `npm run check` and the real generated-preset OMP smoke before handoff
