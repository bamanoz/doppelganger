...
## 8. Runtime Preset opt-in

- [x] 8.1 Add a narrow `evolving-profile` fixture trait that does not duplicate identity, engineering, concise, or user-memory content
...
- [x] 8.3 Update generated temporary preset fixtures and vertical scenarios without making Persona Authoring a generic runtime requirement
- [x] 8.4 Verify full-stack tests create all writable Persona assets under temporary roots rather than consuming personal runtime state
- [x] 8.5 Start a fresh OMP test session and verify the generated preset exposes inspect/revise while identity and undeclared traits remain read-only
...
- [x] 10.3 Document generic Runtime Preset setup, skill installation, OMP/DSH invocation syntax, dry-run, approval, conflict, and recovery outcomes without adding a host-specific command
