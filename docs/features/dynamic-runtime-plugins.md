...
The `doppelganger-capability-evolution` skill may select this workflow only for reversible behavior owned by the current Runtime Session and supported by inspected catalog contracts. Research consent or an Evolution state transition never authorizes generated code. The runtime-plugin skill's inspection, trust warning, immutable Package metadata, lifecycle, and separate native approval gates remain mandatory. Persistent behavior, dependency installation, permanent product code, Runtime Preset composition, and host Client UI route elsewhere.
