...
`servers` is required. The optional root `startupMode` accepts `background` or `await-ready` and defaults to `background`; unknown values fail with the rest of complete synchronous configuration normalization before service publication or external acquisition. The selected mode applies only when that Loader plugin instance is initially applied. Server IDs and configured aliases are lowercase kebab-case. Servers and tools default to enabled; `enabled: false` prevents a server connection, excludes it from initial readiness, or omits the exact configured MCP tool. `startupTimeoutMs` is optional per server, defaults to 60000, and accepts safe integers from 1 through 600000 milliseconds. Unknown fields, relative `cwd` values, non-HTTP endpoints, URL credentials or fragments, malformed environment references, unsupported approval policies, blank or oversized supplied approval reasons, and out-of-range values fail synchronously before any external connection. `approval: { policy: required }` is sufficient; an optional non-empty `reason` is preserved as advisory descriptor metadata.

`background` preserves non-blocking activation and operational failure containment. `await-ready` is an opt-in strict initial-application policy: every enabled initial generation must initialize, negotiate the tools capability, finish validated paginated discovery, atomically commit its complete owner set, and remain current and `active` before the Loader row can activate. Disabled servers do not participate, an empty enabled set is immediately ready, and a valid zero-tool server counts as ready after its empty set commits.
...
After complete synchronous configuration validation, the Loader plugin publishes `doppelgangerMcp`, installs one stable empty registry owner per enabled server, and starts every initial generation concurrently. The service is available while generations are `connecting`, and one server may publish independently before another. There is no cross-server atomic catalog publication promise.

In default `background` mode the plugin returns without awaiting external work. One slow, missing, or invalid external server neither blocks another server nor invalidates the Loader Fiber or Runtime Session. In `await-ready` mode the plugin awaits generation-stable terminal startup outcomes rather than treating fulfillment of cleanup-owning `start()` promises as success. Before returning it revalidates every captured initial generation as the same current active generation; failure, timeout, cancellation, disposal, or replacement rejects activation with credential-safe aggregate diagnostics. Ordinary Loader and Composition Runtime ownership then exhaustively withdraws already-published sibling sets and disposes every attempted-session resource before the failed activation settles.

One generation-owned startup deadline spans transport start, MCP initialization, required tools-capability negotiation, paginated initial `tools/list`, schema validation, candidate construction, and atomic initial commit eligibility in either mode. Schema compilation follows the MCP SDK's draft-07-compatible AJV validator, including an explicit `http://json-schema.org/draft-07/schema#` declaration used by official reference servers. Pagination is bounded to 1,000 pages and repeated cursors fail discovery. A successful generation atomically replaces only its empty owner set and becomes `active`; a failure becomes terminal `failed`, leaves that set empty, closes reachable transport/process resources, records a stage-specific credential-safe diagnostic, and is not retried automatically. The startup deadline bounds the readiness decision and prevents late publication; exhaustive owned cleanup is still awaited and is not represented as completing within that deadline.
...
A valid in-place Loader update retains byte-equivalent normalized server configurations. Changed and removed generations become stale and withdraw their tools before cleanup; removed slots disappear, while changed and added slots immediately install new `connecting` generations and start one tracked background operation each. Update returns after this local cutover without awaiting external connection success, even when the initially applied row selected `await-ready`. Changing only `startupMode` does not retroactively create a barrier or reconnect retained generations; the new value governs a future fresh application or recreation of the row. An unavailable command, endpoint, credential value, or protocol after valid normalization is committed operational `failed` state and does not restore the retired configuration. Structurally invalid authored configuration still rejects synchronously and preserves the prior Loader generation through Composition Runtime rollback. Tool-list notification refresh remains serialized background lifecycle work.

Every generation moves monotonically through `connecting`, `active`, `failed`, and `disposed`; disposal is idempotent from every state. A generation-stable terminal startup outcome is separate from its cleanup promise, so strict readiness can fail promptly while ordinary ownership continues awaiting all reachable cleanup. The Loader Fiber and runtime track startup, retirement cleanup, notification refreshes, active calls, registrations, clients, transports, and child processes. Retirement removes current-generation identity before cancellation, so stale startup, refresh, close, or invocation continuations cannot mutate a replacement or satisfy an old readiness barrier. Row or Runtime Session disposal withdraws complete tool sets, aborts startup and active requests, closes every reachable transport/process, observes background settlements, and reports aggregate cleanup failure only after exhaustive attempts.
...
- `packages/extension-mcp/src/config.ts` — strict Loader configuration, initial startup mode, startup timeout, and credential references.
- `packages/extension-mcp/src/client.ts` — MCP transport, bounded initial startup, generation-stable terminal outcomes, diagnostics, discovery, naming, invocation, refresh, cancellation, and generation disposal.
- `packages/extension-mcp/src/runtime.ts` — independent server slots, strict initial-generation barrier, empty owner registrations, atomic commits/withdrawal, background in-place update cutover, diagnostics, settlement tracking, and aggregate disposal.
- `packages/extension-mcp/src/plugin.ts` — early Cordis service publication, optional awaited initial readiness, background update semantics, and lifecycle/disposal ownership.
- `packages/extension-mcp/tests/config.spec.ts` — configuration-boundary evidence.
- `packages/extension-mcp/tests/integration.spec.ts` — stdio and Streamable HTTP background and await-ready activation, timeout, failure containment, refresh, reload, cancellation, stale-generation, and cleanup evidence.
